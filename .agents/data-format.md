...
| `data/research/diagnostics.json` | `.json` | research-call diagnostics ring (array of diagnostic records, internal state — not a contract, excluded from validation). Each record's **`cost`** (real billed Perplexity USD) is also summed by the **evaluation cost model** (`src/lib/eval/cost-model.ts`, server assembler `src/lib/server/cost.ts`) — the metered-API line of the cost-aware go/no-go scorecard. |
...
**Proposal `catalystSources` (catalyst-news-sources M1).** A `TradeProposal` (and
each `ProposalLensSchema` breakdown) carries **`catalystSources`** — an array of
`CatalystSourceSchema` (`headline`, `publisher`, `url`, `publishedAt`), the
headlines that informed the named catalyst, kept so the catalyst is **verifiable**.
Catalyst capture is now **multi-source with a retry/fallback chain**
(`src/lib/server/catalyst-capture.ts`): **Alpaca's News API** (`getStockNews` in
`alpaca.ts` — free, Benzinga-powered, no daily cap) is the **primary** catalyst
source and the source of these attributions; **Perplexity's** curated `catalysts[]`
phrases (from the capped value research fetch) are the **fallback**. A single
source failing **never** yields "no catalyst" — the chain falls through (the news
fetch is retried, then abandoned to the next source). The pure
`extractCatalystFromNews` (`src/lib/catalyst-news.ts`) scans headlines for a
material why-now (regulatory/FDA/EMA, M&A, earnings, guidance, analyst actions,
product/clinical, Medicare/policy), vetoes listicle/market-wrap noise, and lists
the contributing headlines as sources (capped). The sources are **shared across
both lenses** (it is the symbol's news), **briefed to the red-team**
(`catalystSourcesBriefing` in `red-team.ts` — "the catalyst is backed by these
...
from the structured **`catalysts[]`** why-now phrases (`extractCatalyst` in
`src/lib/catalyst-extract.ts`, pure + unit-tested) — **not** the AI narrative
summary, which is a company-profile blurb that used to green-check the catalyst
item as boilerplate. A phrase that reads like a description (`isCompanyDescription`)
is rejected, and the kept phrase is classified into a `catalystType`
(`classifyCatalyst`) and **word-truncated** (`truncateOnWord` in `src/lib/truncate.ts`)
so it never cuts mid-word. When nothing real is available the catalyst is **`null`**
→ flagged ⚑, rather than a passing description. Text shown/exported (thesis /
research / catalyst) is word-boundary-truncated everywhere, never mid-word.
