...
- **Perplexity adapter:** calls the **Agent API** (`POST /v1/agent` with `tools:[{type:"finance_search"}]`, model `perplexity/sonar`, `max_steps=1`, small `max_output_tokens`) for fundamentals / earnings / analyst / catalyst context — **not** the Sonar chat-completions endpoint. Normalize the structured `finance_results` output to JSON the research routine ingests. (See `planning/fix-perplexity-finance-search-spec.md`.)
